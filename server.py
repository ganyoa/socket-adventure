...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def connect(self):
        """
        method called to wait for a client to connect to the server
        """
    ...
    def room_description(self, room_number):
        ...
        return [
                "You are in the lobby, center of the building.",
                "You are in room one facing West.",
                "You are in room two facing East.",
                "You are in room three facing North."
                ][room_number]
    ...
    def route(self):
        """
        Examines `self.input_buffer` to perform the correct action/method (move, quit, or
        ...
        received = self.input_buffer.split(" ") # break input_buffer into a list of words

        command = received.pop(0).lower().strip() # take the first word of the rcvd msg
        arguments = " ".join(received).strip()

        # keys are strings and values are are references to this instance's methods
        # if command is quit the dict key pull the method self.quit
        {
            'quit': self.quit,
            'move': self.move,
            'say': self.say,
        }[command](arguments) # example result: move(west)
    ...
    def serve(self):
        """
        main Server class loop
        instantiated in serve.py
        the loop connects to a client, greets the client, pushes the greeting to client
        """
        self.connect()
        self.greet()
        self.push_output()

        # loop until done is true in the quit method
        while not self.done:
            self.get_input()
            self.route()
            self.push_output()
